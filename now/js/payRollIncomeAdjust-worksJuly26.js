...
function incomeMaths(index,week,nuHours,nuIncome){
	...
	// console.log('wk1x and wk2x: ',wk1x, wk2x);
	console.log('w1Dx and w2Dx: ',w1Dx,w2Dx);
	
	if ((index !== wk1x)&&(index !== wk2x) ){ // make sure it's a work day, not week total
	// if ((index !== 7)||(index !== 15) ){ // make sure it's a work day, not week total
		// console.log('wk1x and wk2x: ',wk1x, wk2x);

		// console.log('calculate math for work day');
		for (var x =0;x<hrInTotsObj.length;x++){
			if (hrInTotsObj[x].col == index){  // check object if exists with work day
				hrInTotsObj[x].nuHrs = nuHours;	// set added values to object for later recall
				hrInTotsObj[x].nuInc = nuIncome;
		//  ????  no need to check for week1 or week2 ?????
				
				// if (hrInTotsObj[x].week == "week1"){ // for week 1 days
...
function incomeMathWeekTots(index,week,nuHours,nuIncome){
	...
	if ((index !== wk1x)&&(index !== wk2x)){ 
		...
		if (week == 0){ // week 1, this value directly from html click button
			...
			// console.log('adjusted hour, income: ',nuHours,nuIncome);
			console.log('new week1 hours is: ',w1HT);
			console.log('new week1 income is: ',w1InT);
			// adjustedValuesTots = [w1HT,w1InT];
	...
	// week totals when week totals IS clicked
		...
		for (var x=0;x<hrInTotsObj.length;x++){
			// console.log('w2HT and w2InT is: ',w2HT,w2InT)
	...
	// console.log('click array:', clicked);
}
...
function revertMathWeekTots(index,week){
	...
	// console.log('nuHours: ', hrInTotsObj[y].nuHours, 'nuIncome: ',hrInTotsObj[y].nuIncome);
	console.log('w1HT is: ',w1HT, 'w1InT is: ',w1InT);
	console.log('w2HT is: ',w2HT, 'w1InT is: ',w2InT);
		...
		if (week == 0){ // week 1, this value directly from html click button
			for (var x=0;x<hrInTotsObj.length;x++){
				if (hrInTotsObj[x].col == index){ // make sure index matches with object content
					...
					w1HT = w1HT - hrInTotsObj[x].nuHrs; // subtract added hours to current week total
					// w1HT = adjustedValuesTots[0] - hrInTotsObj[x].nuHrs; // subtract added hours to current week total
					w1InT = w1InT - hrInTotsObj[x].nuInc; // subtract added income to current week total
					// w1InT = adjustedValuesTots[1] - hrInTotsObj[x].nuInc; // subtract added income to current week total
			...
			for (var x=0;x<hrInTotsObj.length;x++){
				if (hrInTotsObj[x].col == index){ // make sure index matches with object content
					...
					w2HT = w2HT - hrInTotsObj[x].nuHrs; // subtract added hours to current week total
					// w2HT = adjustedValuesTots[0] - hrInTotsObj[x].nuHrs; // subtract added hours to current week total
					w2InT = w2InT  - hrInTotsObj[x].nuInc; // subtract added income to current week total
					// w2InT = adjustedValuesTots[1] - hrInTotsObj[x].nuInc; // subtract added income to current week total
					console.log('adjusted hour, income: ',hrInTotsObj[x].nuHrs, hrInTotsObj[x].nuInc);
					// console.log('adjusted hour, income: ',nuHours,nuIncome);
					console.log('new week2 hours is: ',w2HT);
					console.log('new week2 income is: ',w2InT);
		...
		for (var x = 0;x<hrInTotsObj.length;x++){
			if (index == clicked[x].ndx){ // make sure button matches object content
				if (clicked[x].click == 1){
					clicked[x].click = 0;
					revertAlreadyYN = 'no';
					// console.log('index no longer clicked');
...
function checkColour(index, week){
	...
	// console.log('check clicks: ',clicked);
	// console.log('w1Dx is: ',w1Dx, ' w2Dx is: ', w2Dx);
	// console.log('anyChangesWk1: ',anyChangesWk1, ' anyChangesWk2: ',anyChangesWk2);
	...
	// console.log('ck>0, this many clicks: ',ck);
// week1
